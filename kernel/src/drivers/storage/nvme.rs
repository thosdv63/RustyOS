...
struct CqEntry {
    ...
    sq_id: u16,        
    command_id: u16,  
    status: u16,       // (bit 0 = phase, high bits = error code)
}
...
impl NvmeDevice {
    ...
    unsafe fn setup_prp(&mut self, pages: &[u64]) -> Result<(u64, u64, Option<u64>), &'static str> {
        ...
        match pages.len() {
            ...
            _ => { // three or more page
                let prp_list_frame = pfa::alloc_frame().ok_or("no frame for prp list")?;
                core::ptr::write_bytes(prp_list_frame as *mut u8, 0, 4096); // safety
                let prp_list_ptr = prp_list_frame as *mut u64;
...
pub fn init(devices: &[PciDevice]) -> Result<(), &'static str> {
    ...
    unsafe {
        ...
        let lbaf: u32 = core::ptr::read_volatile((identify_buf + 128 + (lba_format_index as u64) * 4) as *const u32);
    ...
    unsafe {
        ...
        for i in 0..page_count {
            if let Some(frame) = pfa::alloc_frame() {
                ...
                // Bellek yetmezse o ana kadar ayırdıklarımızı geri ver
                for j in 0..i {
